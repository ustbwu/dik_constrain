...
for j in range(1,n_step+1):
        ...
        sample_joint_hm = [(tensor.detach().numpy().squeeze(0)) for tensor in joint_hm]
        done = collision(sample_joint_hm,d3)
        row_data = done[1]+ output_denoise[0].tolist()+ x_pre[0].tolist()
...
existing_df_con = existing_df.iloc[7000:7001, :6]
...
plt.tight_layout()
plt.savefig('D:\\taiche\\hole_tracky\\ddim_inverse_constrain\\error_distribution.png',dpi=600)
