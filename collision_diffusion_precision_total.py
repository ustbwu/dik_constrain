...
DM.load_state_dict(torch.load(r'D:\\taiche\\hole_tracky\\ddim_inverse_constrain\\loss_dir\\loss_pkl\\loss_336.pkl'))   #??
optimizer.load_state_dict(torch.load(r'D:\\taiche\\hole_tracky\\ddim_inverse_constrain\\loss_dir\\optimizer_pkl\\loss_336.pkl'))   #??
...
for j in range(1,n_step+1):
        ...
        collision_sample = []
        for c in range(len(x_pre)):
            sample_joint_hm = []
            for tensor in joint_hm:
                matrix = tensor[c].detach().cpu().numpy()
                sample_joint_hm.append(matrix)
            collision_sample.append(sample_joint_hm)

        for i in range(len(x_pre)):
            col_dh = collision_sample[i]
            pole = d3[i].detach().cpu().numpy()
            done = collision(col_dh, pole)
            row_data = done[1] + new_joint[i].tolist()
...
start_point_mean = np.mean(start_point)
end_point_mean = np.mean(end_point)
existing_csv_path = 'D:\\taiche\\data_craete\\new_NN\\data\\standard_initial10 - 副本.csv'
existing_df = pd.read_csv(existing_csv_path)
existing_df_con = existing_df.iloc[3000:4000, :6]
column_names = ["col_lrwall", "col_ground", "col_body",  "col_leg","col_arch", "col_self",'joint_1', 'joint_2', 'joint_3', 'joint_4', 'joint_5', 'joint_6', 'joint_7', 'joint_8']
joint_2d = np.array(col_list)
...
output_file_path = f'D:\\taiche\\hole_tracky\\ddim_inverse_constrain\\collision\\{current_datetime}.csv'
...
print('起始点平均误差',start_point_mean)
print('末端点平均误差',end_point_mean)